...
class DatabaseLiveTests: DatabaseTestCase {
  ...
  func testInsertingFavorite() throws {
    ...
    let createdFavorite = try self.database.insertFavorite(
      .init(userId: createdUser.id, description: "foo")
    )
    .run.perform().unwrap()
  ...
  func testFetchingFavorite() throws {
    ...
    let createdFavorite2 = try self.database.insertFavorite(
      .init(userId: createdUser2.id, description: "bar")
    )
    .run.perform().unwrap()
  ...
  func testUpdatingFavorite() throws {
    ...
    let update = DatabaseClient.UpdateFavoriteRequest(
